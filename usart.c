...
bool USART_PutChar(char c){
	...
	bool wynik = RingBuffer_PutChar(&USART_RingBuffer_Tx,c);
...
size_t USART_WriteData(const void *data, size_t dataSize){
	...
	{
		USART_PutChar(((const char*)data)[z]);
	}
...
size_t USART_ReadData(char *data, size_t maxSize){
	...
	for(int i = 0; i<maxSize; i++){
		if(RingBuffer_IsEmpty(&USART_RingBuffer_Rx) == false){
		USART_GetChar(data+i);
		}
	}
	return i;
}
...
char znak;
    ...
    if (__HAL_USART_GET_IT_SOURCE(&UartHandle, USART_IT_TXE)) {
	    // the TXE interrupt is enabled
			RingBuffer_GetChar(&USART_RingBuffer_Tx, &znak);
			USARTx->DR = znak;
			if ( RingBuffer_IsEmpty(&USART_RingBuffer_Tx ==  true) 
